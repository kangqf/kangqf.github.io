...
cp sunxi-boards/sys_config/a20/BananaPro.fex build/BananaPro.fex
# TODO : patch Bananapi.fex here
sunxi-tools/fex2bin build/BananaPro.fex build/script.bin
...
echo "#####################"
echo
dd if=/dev/zero of=${IMG_FILE} bs=1M count=600
...
sudo dd if=build/u-boot-spl.bin of=${LOOP_DEV} bs=1024 seek=8
sync
...
echo "#######################"
echo
mkimage -C none -A arm -T script -d boot.cmd build/boot.scr
...
sudo cp build/script.bin /mnt/boot
sudo cp build/boot.scr /mnt/boot
